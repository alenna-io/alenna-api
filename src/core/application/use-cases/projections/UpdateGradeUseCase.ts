...
import { UpdateGradeInput } from '../../../application/dtos/projections/UpdateGradeInput';
...
export class UpdateGradeUseCase {
  ...
  async execute(projectionId: string, schoolId: string, paceId: string, input: UpdateGradeInput): Promise<Result<Prisma.ProjectionPaceGetPayload<{}>, DomainError>> {
    try {
      ...
      const updatedPace = await this.projectionRepository.updateGrade(projectionId, paceId, input.grade);
