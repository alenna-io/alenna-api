...
export class GetSubjectAndNextLevelsWithPacesUseCase {
  ...
  async execute(subjectId: string): Promise<Result<Prisma.SubjectGetPayload<{ include: { paces: true } }>[], DomainError>> {
    try {
      ...
      return Ok(subjects as Prisma.SubjectGetPayload<{ include: { paces: true } }>[]);
    } catch (error) {
